...
def record_audio(duration=0.5, samplerate=16000):
    """Record audio for voice detection"""
    try:
...
def classify_audio(audio: np.ndarray, threshold=0.02) -> str:
    """Classify audio as voice or noise"""
    if len(audio) == 0:
        return "noise"
...
def voice_detection_thread(voice_queue, stop_event):
    """Continuous voice detection in separate thread"""
...
def simple_test_main():
    """Test head pose, eye tracking, and voice detection without graph system"""
    ...
    print("📹 Testing head pose, eye tracking, and voice detection.")
    ...
    state = {
        ...
        "voice_data": {
            "voice_counts": {"voice": 0, "noise": 0, "error": 0},
            "total_samples": 0
        },
        "start_time": time()
    }
    
    # Set up voice detection thread
    voice_queue = Queue()
    ...
    try:
        while True:
            ...
            # Check for voice detection results
            while not voice_queue.empty():
                voice_result = voice_queue.get()
                current_voice_status = voice_result
                if voice_result in state["voice_data"]["voice_counts"]:
                    state["voice_data"]["voice_counts"][voice_result] += 1
                state["voice_data"]["total_samples"] += 1
            ...
            if frame_count % 5 == 0:
                ...
                print(f"Frame {frame_count}: Head={head_direction}, Eye={eye_direction}, Voice={current_voice_status}")
            ...
            # Voice stats on bottom left
            y_pos = frame.shape[0] - 80
            cv2.putText(frame, f"Voice Samples: {state['voice_data']['total_samples']}", 
                       (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 255), 2)
            y_pos += 25
            
            for status, count in state["voice_data"]["voice_counts"].items():
                cv2.putText(frame, f"V-{status}: {count}", (10, y_pos), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
                y_pos += 20
            
            # Current voice status indicator
            voice_color = (0, 255, 0) if current_voice_status == "voice" else (128, 128, 128)
            cv2.putText(frame, f"Voice: {current_voice_status.upper()}", 
                       (frame.shape[1] - 200, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, voice_color, 2)
            ...
            if current_time - last_print_time >= 10:
                ...
                print(f"Voice: {state['voice_data']['voice_counts']}")
                last_print_time = current_time
            
            cv2.imshow("Simple Test - Head, Eye & Voice Tracking", frame)
        ...
        print(f"Voice tracking samples: {state['voice_data']['total_samples']}")
        print(f"Voice detections: {state['voice_data']['voice_counts']}")
